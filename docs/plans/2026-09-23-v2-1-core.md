...
| `src/tefa.js` (новый) | 6 | `drawTefa(ctx, x, y, r, pose)` — порт утверждённого варианта из `tools/design_tefa.js` |
...
(async () => {
  ...
  for (const p of rows) assert.strictEqual(p.y, -p.row * d.ROW_H);
...
- Produces: `drawTefa(c, x, y, r, pose)` и `TEFA_VARIANT` (объект параметров выбранного варианта).
  ...
  `berserk` — тёплое свечение вокруг и сердитые брови, `alpha` — прозрачность (мигание неуязвимости), `flag` — рисовать зубочистку с флажком.
- Геометрия: `(x, y)` — центр столкновений, `r` — радиус столкновений. Нижняя точка силуэта ровно на `y + r` (стоит на платформе), верх на `y − r·1.16`, ширина `≈ 2r·0.92` (силуэт E выше, чем шире). Вся отрисовка — примитивы Canvas 2D, без `measureText`, без паттернов и фильтров.

- [ ] **Step 1: Прочитать `tools/design_tefa.js`** — функции `bodyPath/drawBody/drawFace/GARNISH.flag` и объект варианта E раунда 1 (тот же, что вариант A раунда 3). Если владелец выбрал другой вариант листа 3, портировать его: отличия только в объекте параметров (`eyes`, `garnish`, палитра, `sx/sy/eggK`), код тот же.
...
```js
'use strict';
// ---------- Тефа: отрисовка утверждённого варианта (лист 3, вариант A = E листа 1) ----------
// Порт из tools/design_tefa.js: тело, крапинки, блик, лицо, флажок. Позы — трансформацией, не по точкам.
...
function drawTefa(c, x, y, r, pose = {}) {
  ...
  if (pose.flag !== false) { /* зубочистка с флажком с макушки */ }
  c.restore();
}
...
```js
// node tools/shot_tefa.js — shots/tefa_port.png: порт drawTefa в размерах 26/54/86 и позах рядом; сравнивать глазами с shots/design_tefa_round3.png (вариант A)
...
- [ ] **Step 4: Проверка.** `node tools/design_tefa.js round3` и `node tools/shot_tefa.js`; ПОСМОТРЕТЬ `shots/tefa_port.png` и `shots/design_tefa_round3.png` (вариант A) — силуэт, палитра, крапинки, веки и флажок совпадают; сквош идёт от низа; при `r = 26` читаются силуэт, блик и лицо. В `tools/test_render.js` в начале цикла по размерам добавить строку `d.drawTefa(ctx, 100, 100, 40, { sx: 1.1, sy: 0.9, tilt: 0.2, face: 0.5, mouth: 0.5, blink: true, hot: 0.5, berserk: true, alpha: 0.5 });` — не падает на proxy-контексте. `index.html`: `<script src="src/tefa.js"></script>` сразу после `src/ball.js`.
...
- Create: `src/fx.js`
- Modify: `src/core.js` (добавить `rrect`)
