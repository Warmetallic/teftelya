...
**Files:**
- Modify: `src/save.js` (полная замена), `tools/test_save.js` (полная замена), `package.json`, `tools/smoke.js`, `tools/test_game.js` (обрезка старых проверок этажей и рекорда — Step 4)
...
- [ ] **Step 4: обрезка старых тестов (их код удаляется в задаче 8).** `package.json` — из `test` убрать `node tools/test_upgrades.js &&` и `node tools/test_floors.js &&`. `tools/smoke.js` — удалить ОБА блока этажей: первый (комментарий `// этажи: пробитие люка…` и следующий `{ … }` до `'этаж сохранён'); }`) и второй (`{ const g = require('./_env')(ctx);` с `g.store.set('teft_save', … floor: 2, startFloor: 2 …)` и до `assert.strictEqual(d.YG.log.at(-1), 'start'); }` включительно — селектор этажей на титуле). `tools/test_game.js` — в строке «рекорд высоты и падение за нижний край» убрать хвост ` assert.ok(d.save.best >= 499);` (v4 не хранит `best`), остальное в строке оставить.
...
```bash
git add src/save.js tools/test_save.js package.json tools/smoke.js tools/test_game.js
