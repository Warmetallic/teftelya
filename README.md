...
- **Управление**: тап = прыжок вверх. Тап слева/справа от тефтели — прыжок в ту сторону.
  Клавиатура: стрелки, A/D, W, пробел, Enter; Escape закрывает магазин. Тап по боковой зоне на десктопе = тап у края поля.
- **Прыжки — ресурс**: 4 заряда (`JUMPS_BASE`), апгрейд поднимает до 5 и 6 (`maxJumps()`). Каждый прыжок тратит заряд. Еда возвращает:
...
- **Мусор**: волос −1, грязь −2, муха −1 (двигается). Масса ниже 1 — проигрыш.
  При потере массы от тефтели физически отлетают куски фарша. Мусор появляется по высоте (`TRASH_FROM`):
  волосы с 60 м, грязь со 130 м, муха с 220 м; до 60 м только еда.
- **Стены**: удар на скорости > 260 (`WALL_HIT`) отрывает 1 массу.
- **Масса даёт**: множитель монет (`COIN_MULT`: масса 4 → ×1.5, 7 → ×2, 10 → ×3, 14 → ×4); с массы 6 (`HAIR_PROOF`)
  волосы не наносят урона («пф»); инерцию — большая тефтеля хуже слушается и сильнее колышется.
- **Магазин** (`src/upgrades.js`, вход с титула и с результатов): заряды 60 и 300 монет, магнит 80/180/400 монет
  с радиусом 60/90/120 px (`magnetRadius()`, тянет только еду). Экономика черновая, цены только в `UPGRADES`.
...
- **Этажи** (`src/floors.js`): башня размечена по 150 м. На потолке этажа люк — пробивается массой ≥ 3+N (до 15),
  под ним буфет (мясо + макароны). Пробитие: бонус 5+5·N монет, этаж открыт для старта — стартовый этаж сам
  сдвигается на следующий (на титуле его можно опустить стрелками). Вентиляция: без пробития таймер 8 с копится,
  только пока массы не хватает — с буфетной массой люк просто ждёт. Старт с любого открытого этажа: масса 1 на
  1-м этаже и 1+N дальше (до 12), мяч стоит на тарелке.

## Тексты
- Персонаж — Тефа, без рода: в русских строках нет гендерных форм («Шлёп!», а не «Упала»).
...
| `src/save.js` | `save {v:3, best, earned, spent, up, floor, startFloor, skin}`, `coins()` = earned − spent; облако + localStorage, при загрузке максимум по полям (все поля монотонны) |
| `src/upgrades.js` | каталог апгрейдов, цены, `maxJumps()`, `magnetRadius()`, `buy()` |
...
| `src/floors.js` | этажи: разметка, люки, буфет, вентиляция, стартовый этаж |
...
| `src/screens.js` | титул, результаты с кнопками, пауза, оверлей рекламы-заглушки, `hitButton`, экран магазина и кнопка «Магазин» с бейджем |
| `src/main.js` | загрузка (`boot`), ввод, пауза от SDK/вкладки/фокуса, реклама между забегами, цикл, состояние shop |
...
- `npm test` — семь тестовых скриптов и smoke: юнит-тесты модулей и `tools/smoke.js` (полный поток экранов на заглушке и на фальшивом `YaGames`).
...
2. ✅ Мета-слой — `docs/specs/2026-09-23-meta-shop-design.md`.
3. Этажи башни — 3a ✅ (`docs/specs/2026-09-23-floors-skins-design.md`), 3b скины и реплики Тефы — следующий.
...
- Вертикальные полосы на телефонах 19.5:9: расширять мир или затемнять как бока?
- Цены магазина — черновые, настроить по метрикам после релиза.
