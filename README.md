...
- **Нижняя граница**: внизу экрана всегда видна смертельная полоса темы (дым, иней, угли, вода, щель под скатертью);
  ...
  Камера (`CAM` в `game.js`) только вверх и привязана к платформе, как в Super Mario World: стоящую Тефу держит на 0.6
  высоты экрана, в полёте стоит — отлететь обратно на свою платформу (даже двойным прыжком строго вверх) не смерть. Если
  вершина прыжка уйдёт выше 0.12 экрана, камера заранее поднимается к ней. Всё движение — через демпфер (SmoothDamp,
  0.3 с): без рывков. `?camdbg` в адресе — ползунки этих трёх чисел для плейтеста.
