...
| `src/i18n.js` | строки ru/en/tr, `T(key)`; `ru` → русский, `tr` → турецкий, всё остальное → английский |
...
| `src/save.js` | `save {v:2, best, earned, spent, up}`, `coins()` = earned − spent; облако + localStorage, при загрузке максимум по полям (все поля монотонны) |
...
- `npm test` — шесть тестовых скриптов и smoke: юнит-тесты модулей и `tools/smoke.js` (полный поток экранов на заглушке и на фальшивом `YaGames`).
