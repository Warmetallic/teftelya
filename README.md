...
- **Этажи** (`src/floors.js`): башня размечена по 150 м. На потолке этажа люк — пробивается массой ≥ 3+N (до 15),
  под ним буфет (мясо + макароны). Пробитие: бонус 5+5·N монет, этаж открыт для старта. Без массы через 8 с —
  вентиляция: проход без награды и без открытия. Старт с любого открытого этажа со стартовой массой 1+N (до 12);
  выбор стрелками на титуле.
