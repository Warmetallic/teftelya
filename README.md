...
- **Опасности**: муха (наводится, предупреждает жужжанием и стрелкой у края 0.7 с, отстаёт через `FLY_CHASE` ≈ 6 с
  ...
  падение за экран.
- **Масса как здоровье**: `massMax()` = `MASS_BASE` 4 + ветка «Мясистость» (без потолка), старт башни с `massMax() − 1`. Удар
  ...
  никакого шара на весь экран. HUD — ряд тефтелек-иконок по числу `massMax()`.
