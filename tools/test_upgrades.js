...
(async () => {
  ...
  for (const k of Object.keys(d.UPGRADES)) assert.strictEqual(d.UP_MAX[k], d.upMaxLevel(k), 'UP_MAX совпадает с каталогом: ' + k);
  assert.strictEqual(d.JUMPS_BASE, 4);
  assert.strictEqual(d.maxJumps(), 4); assert.strictEqual(d.magnetRadius(), 0);
  assert.strictEqual(d.upPrice('jumps'), 60); assert.strictEqual(d.upPrice('magnet'), 80);
  ...
  assert.strictEqual(d.upLevel('jumps'), 1); assert.strictEqual(d.save.spent, 60); assert.strictEqual(d.coins(), 40);
  ...
  assert.ok(!d.canBuy('jumps'), '40 монет на 300 не хватает'); assert.ok(!d.canBuy('magnet'), '40 < 80');
  assert.strictEqual(d.buy('magnet'), false, 'отказ ничего не меняет'); assert.strictEqual(d.save.spent, 60);
  assert.strictEqual(d.upLevel('magnet'), 0, 'отказ не меняет уровень');
  d.save.earned = 2000;
  ...
  assert.strictEqual(d.save.spent, 60 + 80 + 180 + 400 + 300); assert.ok(!d.canBuyAny(), 'всё куплено');
