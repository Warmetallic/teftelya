...
(async () => {
  ...
  assert.strictEqual(ball.y + ball.r, d.plates[0].y, 'мяч стоит на тарелке'); assert.strictEqual(ball.mass, 4); assert.strictEqual(d.plates[0].y, -2970);
  ...
  // мяч всегда стоит на тарелке, даже при большой стартовой массе (этаж 11 → масса 12, r ≈ 69, запас старого варианта был 0.88 px)
  d.reset(1); d.state = 'play'; assert.strictEqual(ball.y + ball.r, d.plates[0].y, 'этаж 1: мяч на тарелке');
  d.reset(11); d.state = 'play';
  assert.strictEqual(ball.y + ball.r, d.plates[0].y, 'этаж 11: мяч на тарелке');
  assert.strictEqual(d.floorOf(Math.max(0, -ball.y / 10)), 11, 'высота мяча соответствует этажу 11');
  ...
  d.reset(1); d.state = 'play'; d.spawnHatch(1); assert.strictEqual(d.nextHatchFloor, 2, 'spawnHatch продвигает nextHatchFloor');
  ball.mass = 5; under(d.hatches[0]); d.YG.gameplayStart();
  ...
  assert.strictEqual(d.save.startFloor, 2, 'новое открытие продвигает стартовый этаж');
  ...
  // floorCleared для уже открытого этажа не трогает стартовый этаж (например, откат выбором на титуле)
  d.save.floor = 3; d.save.startFloor = 1; d.floorCleared(1);
  assert.strictEqual(d.save.startFloor, 1, 'startFloor не продвигается повторным floorCleared');
  ...
  d.reset(1); d.state = 'play'; d.spawnHatch(1); ball.mass = 6; d.hatches[0].need = 7; d.save.floor = 0; // масса 6: иммунитет к волосам, а need 7 — чтобы вентиляция копилась
  for (let i = 0; i < 520; i++) { ball.y = d.hatches[0].y + 200; ball.vy = 0; ball.mass = 6; d.update(0.016); } // масса пиннится: иначе случайная еда рядом дотянет её до need и заморозит таймер
  assert.strictEqual(d.hatches.length, 1, 'второй люк того же этажа не появился');
  // runCoins не проверяем: за 8 с рядом с прижатой тефтелей может заспавниться еда — проверяем, что этаж не засчитан
  assert.ok(d.hatches[0].vented, 'вентиляция'); assert.ok(!d.hatches[0].broken); assert.deepStrictEqual(d.run.clearedThisRun, []); assert.strictEqual(d.run.newUnlock, 0); assert.strictEqual(d.save.floor, 0);
  ...
  d.update(0.016); assert.deepStrictEqual(d.run.clearedThisRun, []); assert.ok(!d.hatches[0].broken);
  // вентиляция не копится, если массы уже хватает на пробитие (need по умолчанию для этажа 1 — 4)
  d.reset(1); d.state = 'play'; d.spawnHatch(1); ball.mass = 6;
  for (let i = 0; i < 100; i++) { ball.y = d.hatches[0].y + 200; ball.vy = 0; d.update(0.016); }
  assert.strictEqual(d.hatches[0].underT, 0, 'массы хватает — таймер не копится'); assert.ok(!d.hatches[0].vented);
  // вне зоны под люком таймер не копится
  d.reset(1); d.state = 'play'; d.spawnHatch(1); ball.mass = 6; d.hatches[0].need = 7;
